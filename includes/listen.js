	...
	const axios = require("axios");
                ...
                logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by chand and Devil" : global.config.BOTNAME}`, "[ BOT INFO ]");
	...
	setInterval(checkAndExecuteEvent, tenMinutes/10);
		...
		if(event.type == "message_reaction" && event.senderID == api.getCurrentUserID() && unsendData[event.threadID].data) api.unsendMessage(event.messageID);
            ...
            //THIZ BOT WAS MADE BY MIRAI TEAM. REMADE BY Instagram: @leesiwoo_s