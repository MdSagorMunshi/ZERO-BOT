...
const time = process.uptime(),
  ...
  const timeNow = moment.tz("Asia/Dahaka").format("DD/MM/YYYY || HH:mm:ss");
...
let a = await loadImage(pathAva)
    ...
    ctx.fillStyle = "#FFF"
    ctx.fillText("ZERO COOL", canvas.width / 2 - 660, 1600);
    ctx.save();
    ctx.font = "350px Grandstander";
    ctx.fillStyle = "#000"
    ctx.fillText("ZERO PROJECT", canvas.width / 2 - 660, 1000);
    ...
    ctx.fillStyle = "#000"
    ctx.fillText("L. SIWOO", 1200, 2750);
    ctx.fillStyle = "#FFF"
    ctx.fillText("LeeTheSwioo", 1600, 3150);
    ctx.fillStyle = "#000"
   ctx.fillText("leesiwoo_s", 2000, 3600);
    ctx.beginPath();
